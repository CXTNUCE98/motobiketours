...
<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-pink-50/30 py-8" v-if="isDepartFrom">
        <div class="container mx-auto px-4">
            <!-- Breadcrumb -->
            <nav class="mb-6 flex items-center gap-2 text-sm">
                <NuxtLink to="/tour"
                    class="text-blue-600 hover:text-blue-700 font-medium transition-colors flex items-center gap-1">
                ...
                <span class="text-gray-700 font-medium capitalize">{{ id.replace(/-/g, ' ') }}</span>
            </nav>

            <!-- Header -->
            <div class="bg-white rounded-2xl shadow-lg p-8 mb-8">
                <h1 class="text-3xl font-bold text-gray-800 mb-3">Tour và Giá</h1>
            ...
            <div class="space-y-6">
                <div v-for="item in cityTours" :key="item.id"
                    class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 border border-gray-100">
                        ...
                        <div class="md:col-span-8 flex flex-col justify-between">
                            <div>
                                <NuxtLink :to="`/tour/${item.id}`"
                                    class="text-2xl font-bold text-gray-800 hover:text-blue-600 transition-colors duration-300 block mb-4">
                                    {{ item.title }}
                                </NuxtLink>
                                <div class="space-y-2 text-gray-700 mb-4">
                                    ...
                                    <div class="flex items-start gap-2">
                                        <svg class="w-5 h-5 text-green-600 mt-0.5" fill="currentColor"
                                            viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                    ...
                                    <div class="flex items-start gap-2">
                                        <svg class="w-5 h-5 text-purple-600 mt-0.5" fill="currentColor"
                                            viewBox="0 0 20 20">
    ...
    <!-- Tour Detail View -->
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-pink-50/30 py-8" v-else-if="tour">
        <div class="container mx-auto px-4">
            <!-- Breadcrumb -->
            <nav class="mb-6 flex items-center gap-2 text-sm">
                <NuxtLink to="/tour"
                ...
                <span class="text-gray-400">/</span>
                <span class="text-gray-700 font-medium">{{ tour.title.substring(0, 50) }}...</span>
            </nav>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg p-6">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-4 text-center">
                                ...
                                <div class="font-bold text-gray-800">{{ tour.duration }}</div>
                            </div>
                            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4 text-center">
                                <svg class="w-8 h-8 text-purple-600 mx-auto mb-2" fill="currentColor"
                                    viewBox="0 0 20 20">
                                    <path
                                        d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" />
                                </svg>
                                <div class="text-sm text-gray-600">Nhóm</div>
                                <div class="font-bold text-gray-800">2-10 người</div>
                            </div>
                            <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 text-center">
                                ...
                                <div class="text-sm text-gray-600">Lộ trình</div>
                                <div class="font-bold text-gray-800 text-xs">{{ tour.routes.substring(0, 15) }}...</div>
                            </div>
                            <div class="bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-4 text-center">
                                <svg class="w-8 h-8 text-orange-600 mx-auto mb-2" fill="currentColor"
                                    viewBox="0 0 20 20">
                                    <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
                                    <path fill-rule="evenodd"
                                        d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z"
                                ...
                                <div class="text-sm text-gray-600">Mã tour</div>
                                <div class="font-bold text-gray-800">{{ tourCode }}</div>
                            </div>
                        </div>

                        <!-- Tabs -->
                        <div class="border-b-2 border-gray-100 mb-6">
                            <div class="flex gap-8">
                                <button @click="setActive('overview')"
                                    class="relative pb-4 font-semibold transition-colors duration-300"
                                    :class="activeTab === 'overview' ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800'">
                                    Tổng quan
                                ...
                                <button @click="setActive('contact')"
                                    class="relative pb-4 font-semibold transition-colors duration-300"
                                    :class="activeTab === 'contact' ? 'text-blue-600' : 'text-gray-600 hover:text-gray-800'">
                                    Liên hệ
                        ...
                        <div class="prose max-w-none">
                            <div v-if="activeTab === 'overview'" v-html="overviewHtml" class="leading-relaxed"></div>
                            <div v-else v-html="contactHtml" class="leading-relaxed"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <NuxtLink v-for="relatedTour in relatedTours" :key="relatedTour.id" :to="`/tour/${relatedTour.id}`"
                        ...
                        <div
                            ...
                            <div class="p-5">
                                <h3
                                    class="text-lg font-bold text-gray-800 mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors duration-300">
                                    {{ relatedTour.title }}
                                </h3>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">{{ relatedTour.duration }}</span>
                                    <span
    ...
    <!-- Not Found -->
    <div v-else
        class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-pink-50/30 flex items-center justify-center py-20">
        <div class="text-center">
            ...
            <h2 class="text-3xl font-bold text-gray-800 mb-4">Không tìm thấy tour</h2>
            <p class="text-gray-600 mb-8">Tour bạn đang tìm kiếm không tồn tại hoặc đã bị xóa.</p>
            <NuxtLink to="/tour"
