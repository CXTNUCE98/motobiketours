...
<template>
  <div class="min-h-screen">
    ...
    <div class="relative overflow-hidden mb-16">
      <!-- Animated Background -->
      <div class="absolute inset-0 bg-gradient-to-br from-[#FF6B35] via-[#FFA726] to-[#1A71C7] opacity-10">
      ...
      <div class="relative z-10 text-center py-20 px-4">
        ...
        <div class="mb-6 animate-fade-in-down">
          <h1 class="text-6xl md:text-7xl font-black mb-4 bg-gradient-to-r from-[#FF6B35] via-[#FFA726] to-[#1A71C7] bg-clip-text text-transparent drop-shadow-lg">
            Latest News & Stories
          </h1>
          <div class="flex items-center justify-center gap-3 text-gray-600 text-lg">
        ...
        <div class="max-w-2xl mx-auto mt-8 animate-fade-in-up">
          <div class="relative group">
            <i class="bx bx-search absolute left-5 top-1/2 -translate-y-1/2 text-2xl text-gray-400 group-focus-within:text-[#FF6B35] transition-colors"></i>
            <input 
              v-model="searchQuery"
              type="text" 
              placeholder="Search for news, destinations, bikes..." 
              class="w-full pl-14 pr-6 py-4 rounded-full border-2 border-gray-200 focus:border-[#FF6B35] focus:ring-4 focus:ring-[#FF6B35]/20 outline-none transition-all duration-300 text-lg shadow-lg backdrop-blur-sm bg-white/90"
            />
          </div>
        </div>
      ...
      <div class="absolute bottom-10 right-10 w-32 h-32 bg-[#1A71C7] rounded-full opacity-20 blur-3xl animate-pulse animation-delay-1000"></div>
    </div>

    <!-- Featured News (First Item) -->
    <div v-if="featuredNews" class="max-w-7xl mx-auto px-4 mb-16 animate-fade-in">
      <div class="relative group overflow-hidden rounded-3xl shadow-2xl hover:shadow-[#FF6B35]/20 transition-all duration-500">
        ...
        <div class="absolute bottom-0 left-0 right-0 p-8 md:p-12 text-white">
          <div class="flex items-center gap-3 mb-4">
            <span class="px-4 py-2 bg-gradient-to-r from-[#FF6B35] to-[#FFA726] rounded-full text-sm font-bold shadow-lg">
          ...
          <router-link 
            ...
            class="inline-flex items-center gap-3 bg-white text-gray-900 px-8 py-4 rounded-full font-bold text-lg shadow-xl hover:shadow-2xl hover:bg-[#FF6B35] hover:text-white transition-all duration-300 transform hover:scale-105 group/btn"
          >
        ...
        <!-- Decorative Corner -->
        <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-[#FFA726] to-transparent opacity-30 blur-2xl"></div>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 mb-16">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div
          v-for="(n, index) in regularNews"
          :key="n.id"
          class="group animate-fade-in-up"
          :style="{ animationDelay: `${index * 100}ms` }"
        >
          <!-- Card Container -->
          <div class="relative h-full bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-[#FF6B35]/10 transition-all duration-500 transform hover:-translate-y-2">
            <!-- Image Section -->
            <div class="relative h-56 overflow-hidden">
              ...
              <div class="absolute top-4 left-4">
                <span class="px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-bold text-[#FF6B35] shadow-lg">
            ...
            <div class="p-6">
              ...
              <!-- Title -->
              <router-link :to="`/news/${n.id}`">
                <h3 class="text-xl font-bold text-gray-800 mb-3 line-clamp-2 group-hover:text-[#FF6B35] transition-colors duration-300">
                  {{ n.title }}
                </h3>
              </router-link>
            ...
            <div class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#FF6B35] via-[#FFA726] to-[#1A71C7] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
          </div>
        </div>
      </div>
      ...
      <div v-if="filteredNews.length === 0" class="text-center py-20">
        <i class="bx bx-search-alt text-8xl text-gray-300 mb-4"></i>
    ...
    <!-- Newsletter Section -->
    <div class="max-w-7xl mx-auto px-4 mb-16">
      <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#FF6B35] via-[#FFA726] to-[#1A71C7] p-12 text-white shadow-2xl">
        <!-- Background Pattern -->
        <div class="absolute inset-0 opacity-10">
          <div class="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] animate-pulse"></div>
        </div>
        ...
        <div class="relative z-10 text-center max-w-3xl mx-auto">
          ...
          <div class="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto">
            ...
            />
            <button class="px-8 py-4 bg-white text-[#FF6B35] rounded-full font-bold hover:bg-gray-100 transition-all duration-300 transform hover:scale-105 shadow-xl">
              Subscribe Now
            </button>
          </div>
        </div>
