...
export default defineEventHandler(async (event) => {
  ...
  if (resendApiKey) {
    ...
    const resp = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.RESEND_FROM || 'Andago Tours <onboarding@resend.dev>',
        to: [toEmail],
        reply_to: body.email,
        subject,
        html,
  ...
  if (!smtpUser || !smtpPass) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      const info = await transport.sendMail({
        from: 'Ethereal <no-reply@example.com>',
        to: toEmail,
        replyTo: body.email,
        subject,
        html,
    ...
    throw createError({
      statusCode: 500,
      statusMessage: 'SMTP credentials are missing. Set SMTP_USER and SMTP_PASS, or use RESEND_API_KEY.',
    });
  }
