...
export function validateTourForm(formData: CreateTourDto): string[] {
  const errors: string[] = [];
  if (!formData.title || formData.title.trim() === '') errors.push('titleRequired');
  if (!formData.type) errors.push('typeRequired');
  if (formData.price_usd === undefined || formData.price_usd === null) errors.push('priceRequired');
  if (!formData.duration) errors.push('durationRequired');
  if (!formData.depart_from) errors.push('departFromRequired');
  if (!formData.routes) errors.push('routesRequired');
  if (!formData.description) errors.push('descriptionRequired');
  if (!formData.content) errors.push('contentRequired');
  return errors;
}
